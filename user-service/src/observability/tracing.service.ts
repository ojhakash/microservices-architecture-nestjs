...
const { W3CTraceContextPropagator, AsyncHooksContextManager } = require('@opentelemetry/core');
...
export class TracingService implements OnModuleInit, OnModuleDestroy {
  ...
  // Extract trace context from headers and create a linked span
  startSpanFromHeaders(name: string, headers: Record<string, string>, kind = SpanKind.CONSUMER) {
    ...
    if (parentTraceId && parentSpanId) {
      // Create a span with a link to the parent trace
      const span = this.tracer.startSpan(name, {
        ...
        links: [{
          context: {
            ...
            spanId: parentSpanId,
            traceFlags: parseInt(headers['x-trace-flags'] || '1'),
          }
        }],
      });
      span.setAttribute('linked.trace_id', parentTraceId);
      span.setAttribute('linked.span_id', parentSpanId);
      return span;
    }
